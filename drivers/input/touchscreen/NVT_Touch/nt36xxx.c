...
#define ID_GESTURE_WORD_M			17
#define ID_GESTURE_WORD_O			18
...
#define MASK_GESTURE_M 0x200
#define MASK_GESTURE_O 0x240
...
void nvt_ts_wakeup_gesture_report(uint8_t gesture_id, uint8_t *data)
{
	...
	case ID_GESTURE_WORD_Z:
		...
		break;
	case ID_GESTURE_WORD_M:
		if (screen_gesture) {
			NVT_LOG("Gesture : Word-Z.\n");
			keycode = gesture_key_array[5];
		}
		break;
	case ID_GESTURE_WORD_O:
		if (screen_gesture) {
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	__set_bit(GESTURE_EVENT_M, ts->input_dev->keybit);
	__set_bit(GESTURE_EVENT_O, ts->input_dev->keybit);
